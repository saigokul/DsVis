...
class SORTINSERTIONgui:
    def __init__(self, root, sortinsertionobj):
        self.root = root
        self.sortinsertionobj = sortinsertionobj
    ...
    def sort(self):
        ...
        if (len(arr2) != 0):
            ...
            self.sortinsertionobj.array = arr2
            print(self.sortinsertionobj.array)
            self.inputs.clear()
            self.inputs.disable()
            ...
            self.sortinsertionobj.insertion_sort()
            self.back_btn.state(['!disabled'])
            self.sort_btn.state(['!disabled'])
            self.inputs.enable()
    ...
    def setupcanvas(self):
        ...
        self.help = \
            ...
            '   White colored containers represent sorted part.\n'\
            '   Green colored containers represent unsorted part.\n'\
            '   Pale red colored container represent the element which is picked from the unsorted part.\n'
        self.set_info(self.info_tag, self.help, 'overwrite')
    ...
    def dual_blink(self, index1, index2, pass_num, swap=True):
        canvas = self.area
        ...
        if (swap):
            blink_color = '#ff0000' '''
        blink_color = outline_color
        canvas.itemconfigure(tag1.box(), outline=blink_color, width=1)
        canvas.itemconfigure(tag2.box(), outline=blink_color, width=1)
        canvas.update()
        ...
        if (swap):
            ...
            col1 = canvas.itemcget(tag1.box(), 'fill')
            col2 = canvas.itemcget(tag2.box(), 'fill')
            canvas.itemconfigure(tag1.box(), fill=col2)
            canvas.itemconfigure(tag2.box(), fill=col1)
        canvas.update()
        time.sleep(self.anim_time)
        canvas.itemconfigure(tag1.box(), outline=blink_color, width=1)
        canvas.itemconfigure(tag2.box(), outline=blink_color, width=1)
