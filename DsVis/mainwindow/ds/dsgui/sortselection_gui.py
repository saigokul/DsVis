...
class SORTSELECTIONgui:
    def __init__(self, root, sortselectionobj):
        self.root = root
        self.sortselectionobj = sortselectionobj
    ...
    def sort(self):
        ...
        if (len(arr2) != 0):
            ...
            self.sortselectionobj.array = arr2
            print(self.sortselectionobj.array)
            self.inputs.clear()
            self.inputs.disable()
            ...
            self.sortselectionobj.selection_sort()
            self.back_btn.state(['!disabled'])
            self.sort_btn.state(['!disabled'])
            self.inputs.enable()
    ...
    def setupcanvas(self):
        ...
        self.area.create_text(10, 10, anchor='nw', tags=(self.info_tag))
        self.help = \
            'Selection Sort \n\n Note :\n\n   White colored containers represent that the data is processed and it' \
            'is in the correct position.\n   Green colored containers represent the data which is to be processed.\n' \
            '\n   When minimum element is found it gets highlighted in blue color.' \
            '\n   After minimum element is found it gets swapped with the element in first green container.'
        self.set_info(self.info_tag, self.help, 'overwrite')
    ...
    def blink_higlight(self, index1, pass_num, is_min=False):
        canvas = self.area
